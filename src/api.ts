...
/** Сбросить заказы тестовых юзеров (DELETE, только admin). */
export async function resetTestOrders(adminUsername: string): Promise<number | null> {
  try {
    const r = await fetch(`${API_BASE}/orders/test-users`, {
      method: "DELETE",
      headers: authHeaders(adminUsername),
    });
    if (!r.ok) return null;
...
/** Сбросить вообще все заказы (DELETE, только admin). */
export async function resetAllOrders(adminUsername: string): Promise<number | null> {
  try {
    const r = await fetch(`${API_BASE}/orders`, {
      method: "DELETE",
      headers: authHeaders(adminUsername),
